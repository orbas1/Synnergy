...
func (b *BaseToken) Allowance(o, s Address) uint64 {
    if v, ok := b.allowance.Load(o); ok {
        if inner, ok2 := v.(*sync.Map); ok2 {
            if amt, ok3 := inner.Load(s); ok3 { return amt.(uint64) }
        }
    }
    return 0
}
...
func (b *BaseToken) Transfer(from, to Address, amt uint64) error {
    if err := b.ledger.WithinBlock(func() error {
        if err := b.balances.Sub(b.id, from, amt); err != nil { return err }
        b.balances.Add(b.id, to, amt); return nil
    }); err != nil { return err }
...
func getRegistry() *ContractRegistry {
    regOnce.Do(func() {
        reg = &ContractRegistry{
            ledger: nil, // or assign properly
            vm:     nil, // or assign properly
...
func RegisterToken(t Token) {
    r := getRegistry(); r.mu.Lock(); r.tokens[t.ID()] = t.(*BaseToken)
; r.mu.Unlock()
    log.WithField("symbol", t.Meta().Symbol).Info("token registered")
}

func GetToken(id TokenID) (Token, bool) { t, ok := getRegistry().tokens[id]; return t, ok }
...
func (Factory) Create(meta Metadata, init map[Address]uint64) (Token, error) {
    if meta.Created.IsZero() { meta.Created = time.Now().UTC() }
    bt := &BaseToken{id: deriveID(meta.Standard), meta: meta, balances: NewBalanceTable()}
    for a, v := range init { bt.balances.Set(bt.id, a, v); bt.meta.TotalSupply += v }
    RegisterToken(bt); return bt, nil
}
...
func init() {
    ...
    for _, m := range canon {
        if _, err := f.Create(m, map[Address]uint64{AddressZero: 0}); err != nil { panic(err) }
    }
...
Register(0xB0, func(ctx Context) error {
        id := TokenID(ctx.Stack.PopUint32()); to := ctx.Stack.PopAddress(); amt := ctx.Stack.PopUint64(); from := ctx.TxOrigin
        tok, ok := GetToken(id); if !ok { return ErrInvalidAsset }
        if err := tok.Transfer(from, to, amt); err != nil { return err }
        ctx.Stack.PushBool(true); ctx.RefundGas(OpTokenTransfer); return nil
    })
