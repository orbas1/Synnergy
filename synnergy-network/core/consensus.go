...
func (sc *SynnergyConsensus) ProposeSubBlock() (*SubBlock, error) {
    ...
    ts := make([]byte, 8); binary.LittleEndian.PutUint64(ts, uint64(header.Timestamp)); h.Write(ts)
    ...
    if err := sc.ledger.AppendSubBlock(sb); err != nil { return nil, err }
...
func (sc *SynnergyConsensus) handlePoSVote(msg InboundMsg) {
    var vote struct{ HeaderHash []byte; Sig []byte }
    if err := msg.Decode(&vote); err != nil { return }
...
func (sc *SynnergyConsensus) ValidatePoH(sb *SubBlock) error {
    h := sha256.New()
    for _, tx := range sb.Body.Transactions { h.Write(tx) }
    ts := make([]byte, 8); binary.LittleEndian.PutUint64(ts, uint64(sb.Header.Timestamp)); h.Write(ts)
    if !equal(sb.Header.PoHHash, h.Sum(nil)) { return errors.New("PoH mismatch") }
    return nil
}
...
func (sc *SynnergyConsensus) SealMainBlockPOW(headers []SubBlockHeader) error {
    ...
    for {
        ...
        if new(big.Int).SetBytes(hash[:]).Cmp(target) <= 0 {
            bh.Nonce = nonce; bh.PoWHash = hash[:]
            break
        }
        nonce++
    }
    ...
    if err := sc.ledger.AppendBlock(blk); err != nil { return err }
    sc.logger.Printf("block #%d sealed (nonce %d)", bh.Height, nonce)
    sc.recordBlkTime(bh.Timestamp); sc.retargetDifficulty()
...
func (sc *SynnergyConsensus) recordBlkTime(ts int64) {
    ...
    if len(sc.blkTimes) > RetargetWindow { sc.blkTimes = sc.blkTimes[1:] }
}
...
func (sc *SynnergyConsensus) retargetDifficulty() {
    sc.mu.Lock(); defer sc.mu.Unlock()
    n := len(sc.blkTimes); if n < 2 { return }
    ...
    next := new(big.Int); newF.Int(next)
...
func (sc *SynnergyConsensus) nextSubHeightAtomic() uint64 { sc.mu.Lock(); h := sc.nextSubHeight; sc.nextSubHeight++; sc.mu.Unlock(); return h }
func (sc *SynnergyConsensus) nextBlkHeightAtomic() uint64 { sc.mu.Lock(); h := sc.nextBlkHeight; sc.nextBlkHeight++; sc.mu.Unlock(); return h }

//---------------------------------------------------------------------
// Util
//---------------------------------------------------------------------

func equal(a, b []byte) bool { if len(a)!=len(b) { return false }; for i:=range a { if a[i]!=b[i] { return false } }; return true }
