...
// Core networking types such as NodeID, Peer, Message, Config, NetworkMessage,
// Block, NATManager, parsePort and the Node struct itself are defined in other
// files within this package (e.g. common_structs.go and nat_traversal.go). The
// definitions were previously duplicated here which caused redeclaration build
// failures. The duplicates have been removed so that all consumers reference the
