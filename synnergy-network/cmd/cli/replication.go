...
func newReplClient(ctx context.Context) (*replClient, error) {
    ...
    if addr == "" { addr = "127.0.0.1:7950" }
    d := net.Dialer{}
    conn, err := d.DialContext(ctx, "tcp", addr)
    if err != nil { return nil, fmt.Errorf("cannot connect to replication daemon at %s: %w", addr, err) }
...
func statusRPC(ctx context.Context) (map[string]any, error) {
    ...
    defer cli.Close()
    if err := cli.writeJSON(map[string]any{"action": "status"}); err != nil { return nil, err }
    var resp struct{ Data map[string]any `json:"data"`; Error string `json:"error,omitempty"` }
