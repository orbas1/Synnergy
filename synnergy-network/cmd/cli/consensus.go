...
func initConsensusMiddleware(cmd *cobra.Command, _ []string) error {
    ...
    if err != nil { return fmt.Errorf("invalid LOG_LEVEL %s: %w", lvlStr, err) }
    logger.SetLevel(lvl)

    // 3. ledger (singleton)
    lp := os.Getenv("LEDGER_PATH")
    if lp == "" { return fmt.Errorf("LEDGER_PATH not set") }
    if ledger == nil {
        ledger, err = core.OpenLedger(lp)
        if err != nil { return fmt.Errorf("open ledger: %w", err) }
    }

    // Short‑circuit if consensus already initialised
    consensusMu.RLock()
    if consensus != nil { consensusMu.RUnlock(); return nil }
    consensusMu.RUnlock()

    // 4. p2p network service
    listenPort := os.Getenv("P2P_PORT")
    if listenPort == "" { listenPort = "30333" }
    ...
    if err != nil { return fmt.Errorf("init p2p: %w", err) }

    // 5. security (crypto keys & signatures)
    ksPath := os.Getenv("KEYSTORE_PATH")
    if ksPath == "" { return fmt.Errorf("KEYSTORE_PATH not set") }
    secSvc, err := security.NewService(security.Config{KeyStorePath: ksPath})
    if err != nil { return fmt.Errorf("init security: %w", err) }
    ...
    if authDB == "" { return fmt.Errorf("AUTH_DB_PATH not set") }
    authSvc, err := authority.New(authority.Config{DBPath: authDB, Ledger: ledger})
    if err != nil { return fmt.Errorf("init authority: %w", err) }

    // 8. create consensus
    cns, err := core.NewConsensus(logger, ledger, p2pSvc, secSvc, txPoolSvc, authSvc)
    if err != nil { return fmt.Errorf("new consensus: %w", err) }
