...
func signRPC(ctx context.Context, algo, keyHex, msgHex string) (string, error) {
    ...
    if err != nil { return "", err }
    defer cli.Close()
    if err := cli.writeJSON(map[string]any{"action": "sign", "algo": algo, "key": keyHex, "msg": msgHex}); err != nil { return "", err }
    var resp struct{ Sig string `json:"sig"`; Error string `json:"error,omitempty"` }
...
func verifyRPC(ctx context.Context, algo, pubHex, msgHex, sigHex string) (bool, error) {
    ...
    defer cli.Close()
    if err := cli.writeJSON(map[string]any{"action": "verify", "algo": algo, "pub": pubHex, "msg": msgHex, "sig": sigHex}); err != nil { return false, err }
    var resp struct{ Ok bool `json:"ok"`; Error string `json:"error,omitempty"` }
    if err := cli.readJSON(&resp); err != nil { return false, err }
    if resp.Error != "" { return false, errors.New(resp.Error) }
    return resp.Ok, nil
}
...
func aggregateRPC(ctx context.Context, sigs []string) (string, error) {
    ...
    if err != nil { return "", err }
    defer cli.Close()
    if err := cli.writeJSON(map[string]any{"action": "aggregate", "sigs": sigs}); err != nil { return "", err }
    var resp struct{ Agg string `json:"agg"`; Error string `json:"error,omitempty"` }
    if err := cli.readJSON(&resp); err != nil { return "", err }
    if resp.Error != "" { return "", errors.New(resp.Error) }
    return resp.Agg, nil
}
...
func encryptRPC(ctx context.Context, keyHex, msgHex string) (string, error) {
    ...
    if err != nil { return "", err }
    defer cli.Close()
    if err := cli.writeJSON(map[string]any{"action": "encrypt", "key": keyHex, "msg": msgHex}); err != nil { return "", err }
    var resp struct{ Blob string `json:"blob"`; Error string `json:"error,omitempty"` }
    if err := cli.readJSON(&resp); err != nil { return "", err }
    if resp.Error != "" { return "", errors.New(resp.Error) }
...
func decryptRPC(ctx context.Context, keyHex, blobHex string) (string, error) {
    ...
    if err != nil { return "", err }
    defer cli.Close()
    if err := cli.writeJSON(map[string]any{"action": "decrypt", "key": keyHex, "blob": blobHex}); err != nil { return "", err }
    var resp struct{ Msg string `json:"msg"`; Error string `json:"error,omitempty"` }
    if err := cli.readJSON(&resp); err != nil { return "", err }
    if resp.Error != "" { return "", errors.New(resp.Error) }
    return resp.Msg, nil
}
...
func merkleRPC(ctx context.Context, leaves []string) (string, error) {
    ...
    if err != nil { return "", err }
    defer cli.Close()
    if err := cli.writeJSON(map[string]any{"action": "merkle", "leaves": leaves}); err != nil { return "", err }
    var resp struct{ Root string `json:"root"`; Error string `json:"error,omitempty"` }
    if err := cli.readJSON(&resp); err != nil { return "", err }
    if resp.Error != "" { return "", errors.New(resp.Error) }
...
var verifyCmd = &cobra.Command{
    ...
    RunE: func(cmd *cobra.Command, args []string) error {
        ...
        if err != nil { return err }
        if ok { fmt.Println("OK") } else { fmt.Println("FAIL") }
        return nil
    },
}
...
var encCmd = &cobra.Command{
    ...
    RunE: func(cmd *cobra.Command, args []string) error {
        key, _ := cmd.Flags().GetString("key")
        msg, _ := cmd.Flags().GetString("msg")
        if key == "" || msg == "" { return errors.New("--key --msg required") }
        ctx, cancel := context.WithTimeout(cmd.Context(), 2*time.Second)
        defer cancel()
...
var decCmd = &cobra.Command{
    ...
    RunE: func(cmd *cobra.Command, args []string) error {
        ...
        if key == "" || blob == "" { return errors.New("--key --blob required") }
        ctx, cancel := context.WithTimeout(cmd.Context(), 2*time.Second)
        defer cancel()
