...
func headRPC(ctx context.Context) (height uint64, hash string, err error) {
    ...
    if errN != nil { err = errN; return }
    defer cli.Close()
    if err = cli.writeJSON(map[string]any{"action": "head"}); err != nil { return }
    var resp struct{ Height uint64 `json:"height"`; Hash string `json:"hash"`; Error string `json:"error,omitempty"` }
    if err = cli.readJSON(&resp); err != nil { return }
    if resp.Error != "" { err = errors.New(resp.Error); return }
...
func blockRPC(ctx context.Context, h uint64) (*core.Block, error) {
    ...
    defer cli.Close()
    if err := cli.writeJSON(map[string]any{"action": "block", "height": h}); err != nil { return nil, err }
    var resp struct{ Block core.Block `json:"block"`; Error string `json:"error,omitempty"` }
...
func balanceRPC(ctx context.Context, addr string) (map[string]uint64, error) {
    ...
    if err != nil { return nil, err }
    defer cli.Close()
    if err := cli.writeJSON(map[string]any{"action": "balance", "addr": addr}); err != nil { return nil, err }
    var resp struct{ Bal map[string]uint64 `json:"bal"`; Error string `json:"error,omitempty"` }
    if err := cli.readJSON(&resp); err != nil { return nil, err }
    if resp.Error != "" { return nil, errors.New(resp.Error) }
    return resp.Bal, nil
}
...
func utxoRPC(ctx context.Context, addr string, limit int) ([]core.UTXO, error) {
    ...
    if err != nil { return nil, err }
    defer cli.Close()
    if err := cli.writeJSON(map[string]any{"action": "utxo", "addr": addr, "limit": limit}); err != nil { return nil, err }
    var resp struct{ List []core.UTXO `json:"list"`; Error string `json:"error,omitempty"` }
    if err := cli.readJSON(&resp); err != nil { return nil, err }
    if resp.Error != "" { return nil, errors.New(resp.Error) }
...
func poolRPC(ctx context.Context, limit int) ([]core.Transaction, error) {
    ...
    if err != nil { return nil, err }
    defer cli.Close()
    if err := cli.writeJSON(map[string]any{"action": "pool", "limit": limit}); err != nil { return nil, err }
    var resp struct{ List []core.Transaction `json:"list"`; Error string `json:"error,omitempty"` }
    if err := cli.readJSON(&resp); err != nil { return nil, err }
    if resp.Error != "" { return nil, errors.New(resp.Error) }
    return resp.List, nil
}
...
var blockCmd = &cobra.Command{
    ...
    RunE: func(cmd *cobra.Command, args []string) error {
        ...
        if err != nil { return fmt.Errorf("invalid height: %w", err) }
