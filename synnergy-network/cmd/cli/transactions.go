...
import (
    ...
    "github.com/synnergy-network/network"
    "github.com/synnergy-network/security"
    "github.com/synnergy-network/txpool"
    "github.com/synnergy-network/authority"
)
...
func initTxMiddleware(cmd *cobra.Command, _ []string) error {
    ...
    initOnce.Do(func() {
        ...
        if err != nil { retErr = fmt.Errorf("invalid LOG_LEVEL: %w", err); return }
        logger.SetLevel(lvl)

        // 3. Ledger
        lp := os.Getenv("LEDGER_PATH")
        if lp == "" { retErr = fmt.Errorf("LEDGER_PATH not set"); return }
        ledger, err = core.OpenLedger(lp)
        if err != nil { retErr = fmt.Errorf("open ledger: %w", err); return }
        ...
        if err != nil { retErr = fmt.Errorf("init p2p: %w", err); return }

        // 5. Security / keystore
        ks := os.Getenv("KEYSTORE_PATH")
        if ks == "" { retErr = fmt.Errorf("KEYSTORE_PATH not set"); return }
        secSvc, err = security.NewService(security.Config{KeyStorePath: ks})
        if err != nil { retErr = fmt.Errorf("init security: %w", err); return }

        // 6. Authority (for tx reversal checks)
        authDB := os.Getenv("AUTH_DB_PATH")
        if authDB == "" { retErr = fmt.Errorf("AUTH_DB_PATH not set"); return }
        authSvc, err := authority.New(authority.Config{DBPath: authDB, Ledger: ledger})
        if err != nil { retErr = fmt.Errorf("init authority: %w", err); return }
...
func handleSign(cmd *cobra.Command, _ []string) error {
    ...
    if err := json.Unmarshal(raw, &tx); err != nil { return fmt.Errorf("decode tx: %w", err) }
    ...
    if flags.output != "" {
        if err := ioutil.WriteFile(flags.output, jsonBytes, 0o600); err != nil { return err }
...
func handleSubmit(cmd *cobra.Command, _ []string) error {
    ...
    var tx core.Transaction
    if err := json.Unmarshal(raw, &tx); err != nil { return err }
    if err := txPoolSvc.AddTx(&tx); err != nil { return fmt.Errorf("pool reject: %w", err) }
