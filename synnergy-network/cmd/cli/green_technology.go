...
func certificateOfRPC(ctx context.Context, addr string) (core.Certificate, error) {
    ...
    if err != nil { return "", err }
    defer cli.Close()
    if err := cli.writeJSON(map[string]any{"action": "certificate_of", "addr": addr}); err != nil { return "", err }
    var resp struct{ Cert core.Certificate `json:"cert"`; Error string `json:"error,omitempty"` }
...
func shouldThrottleRPC(ctx context.Context, addr string) (bool, error) {
    ...
    defer cli.Close()
    if err := cli.writeJSON(map[string]any{"action": "should_throttle", "addr": addr}); err != nil { return false, err }
    var resp struct{ Throttle bool `json:"throttle"`; Error string `json:"error,omitempty"` }
...
var usageCmd = &cobra.Command{
    ...
    RunE: func(cmd *cobra.Command, args []string) error {
        ...
        if err != nil { return fmt.Errorf("invalid --energy: %w", err) }
        carbon, err := strconv.ParseFloat(carbonStr, 64)
        if err != nil { return fmt.Errorf("invalid --carbon: %w", err) }
...
var offsetCmd = &cobra.Command{
    ...
    RunE: func(cmd *cobra.Command, args []string) error {
        ...
        if kgStr == "" { return errors.New("--kg required") }
        kg, err := strconv.ParseFloat(kgStr, 64)
        if err != nil { return fmt.Errorf("invalid --kg: %w", err) }
        ctx, cancel := context.WithTimeout(cmd.Context(), 2*time.Second)
        defer cancel()
...
var throttleCmd = &cobra.Command{
    ...
    RunE: func(cmd *cobra.Command, args []string) error {
        ...
        if err != nil { return err }
        if yes { fmt.Println("yes") } else { fmt.Println("no") }
        return nil
    },
}
