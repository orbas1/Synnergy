...
func newMockLedger() *mockLedger { return &mockLedger{blocks: make(map[Hash]*Block)} }
func (l *mockLedger) HasBlock(h Hash) bool { l.mu.RLock(); _,ok:=l.blocks[h]; l.mu.RUnlock(); return ok }
func (l *mockLedger) BlockByHash(h Hash) (*Block, error) { l.mu.RLock(); b,ok:=l.blocks[h]; l.mu.RUnlock(); if !ok {return nil, errors.New("not found");}; return b,nil }
func (l *mockLedger) ImportBlock(b *Block) error { h := b.Hash(); l.mu.Lock(); l.blocks[h]=b; l.mu.Unlock(); return nil }
func (l *mockLedger) DecodeBlockRLP(data []byte) (*Block,error){ var blk Block; if err:=rlp.DecodeBytes(data,&blk); err!=nil {return nil,err}; return &blk,nil }
...
func (b *Block) Hash() Hash {
    ...
    var h Hash; copy(h[:], second); return h
}
...
type InboundMsg struct{ PeerID string; Code byte; Payload []byte }

//------------------------------------------------------------
// ReplicationConfig stub
//------------------------------------------------------------

type ReplicationConfig struct{ Fanout float64; RequestTimeout time.Duration }
...
func TestRequestMissing_Success(t *testing.T){
    ...
    h := blk.Hash(); led.ImportBlock(blk)
    ...
    if err!=nil { t.Fatalf("req err %v", err) }
    if got==nil || got.Header.Height!=7 { t.Fatalf("unexpected block returned") }
}
