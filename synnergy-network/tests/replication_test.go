...
func (l *mockLedger) HasBlock(h Hash) bool {
	l.mu.RLock()
	_, ok := l.blocks[h]
	l.mu.RUnlock()
	return ok
}
func (l *mockLedger) BlockByHash(h Hash) (*Block, error) {
	l.mu.RLock()
	b, ok := l.blocks[h]
	l.mu.RUnlock()
	if !ok {
...
func (l *mockLedger) ImportBlock(b *Block) error {
	h := b.Hash()
	l.mu.Lock()
	l.blocks[h] = b
	l.mu.Unlock()
...
func (l *mockLedger) DecodeBlockRLP(data []byte) (*Block, error) {
	var blk Block
