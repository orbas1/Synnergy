...
func genSelfSignedCert(t *testing.T) (certPEM, keyPEM []byte) {
    ...
    if err != nil { t.Fatalf("cert create: %v", err) }
    certPEM = pem.EncodeToMemory(&pem.Block{Type: "CERTIFICATE", Bytes: der})
    privDER, err := x509.MarshalPKCS8PrivateKey(priv)
    if err != nil { t.Fatalf("priv der: %v", err) }
...
func TestSignVerify_Ed25519(t *testing.T) {
    ...
    if err != nil || !ok { t.Fatalf("verify failed err=%v ok=%v", err, ok) }

    // corrupt sig
    sig[0] ^= 0xFF
    ok, _ = Verify(AlgoEd25519, pub, msg, sig)
    if ok { t.Fatalf("expected verify=false on corrupted sig") }
}

func TestSignVerify_Errors(t *testing.T) {
    _, err := Sign(99, nil, nil)
    if err == nil { t.Fatalf("expected unknown algo error") }

    _, err = Sign(AlgoEd25519, "notkey", nil)
    if err == nil { t.Fatalf("expected invalid key type error") }

    _, err = Verify(AlgoEd25519, "badpub", nil, nil)
    if err == nil { t.Fatalf("expected invalid pub error") }
}
...
func TestCombineShares(t *testing.T) {
    ...
    if err != nil { t.Fatalf("combine err %v", err) }
    if !bytes.Equal(got, secret) { t.Fatalf("combine mismatch") }
...
func TestEncryptDecrypt(t *testing.T) {
    ...
    if !bytes.Equal(plain, msg) { t.Fatalf("decrypt mismatch") }
...
func TestComputeMerkleRoot(t *testing.T) {
    if _, err := ComputeMerkleRoot(nil); err == nil { t.Fatalf("want error on empty leaves") }
    a := sha256.Sum256([]byte("a"))
    b := sha256.Sum256([]byte("b"))
    root1, _ := ComputeMerkleRoot([][]byte{a[:], b[:]})
    root2, _ := ComputeMerkleRoot([][]byte{b[:], a[:]}) // order changed, roots equal due to sort
    if !bytes.Equal(root1, root2) { t.Fatalf("root mismatch, sort failed") }
    if len(root1) != 32 { t.Fatalf("root len %d", len(root1)) }
}
...
func TestNewTLSConfig(t *testing.T) {
    ...
    if err != nil { t.Fatalf("tls cfg err %v", err) }
    if cfg.MinVersion != tls.VersionTLS13 { t.Fatalf("min version not TLS1.3") }
}
v