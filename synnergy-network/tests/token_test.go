...
func (m *tokenLedger) EmitApproval(tok TokenID, owner, spender Address, amount uint64) {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *tokenLedger) EmitTransfer(tok TokenID, from, to Address, amount uint64) {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *tokenLedger) DeductGas(addr Address, amount uint64) {
	m.mu.Lock()
	defer m.mu.Unlock()
	m.gasCalls = append(m.gasCalls, amount)
}

func (m *tokenLedger) WithinBlock(fn func() error) error { return fn() }
...
func TestDeriveID(t *testing.T) {
	...
	if got != want {
		t.Fatalf("deriveID wrong: got %x want %x", got, want)
	}
}
...
func TestFactoryAndRegistry(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("factory create err %v", err)
	}
...
func prepareBaseToken(balanceA uint64) (*BaseToken, *tokenLedger) {
	bt := &BaseToken{
