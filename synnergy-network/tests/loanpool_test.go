...
func newLpLedger() *lpMockLedger { return &lpMockLedger{state: make(map[string][]byte), holders: make(map[Address]bool)} }

func (m *lpMockLedger) SetState(k, v []byte) error { m.mu.Lock(); m.state[string(k)] = append([]byte(nil), v...); m.mu.Unlock(); return nil }
func (m *lpMockLedger) GetState(k []byte) ([]byte, error) { m.mu.RLock(); defer m.mu.RUnlock(); return m.state[string(k)], nil }
func (m *lpMockLedger) HasState(k []byte) (bool, error) { m.mu.RLock(); defer m.mu.RUnlock(); _, ok := m.state[string(k)]; return ok, nil }
func (m *lpMockLedger) PrefixIterator(pref []byte) PrefixIterator {
    m.mu.RLock(); defer m.mu.RUnlock(); var kvs []KV; for k,v := range m.state { if len(k)>=len(pref)&&k[:len(pref)]==string(pref){ kvs=append(kvs, KV{k: []byte(k), v:v}) } }; return &sliceIterLP{items:kvs}
}
...
type sliceIterLP struct { items []KV; idx int }
...
type mockElector struct { authSet map[Address]bool; elect []Address; randErr error }
func (m mockElector) RandomElectorate(_ int)([]Address,error){ if m.randErr!=nil {return nil,m.randErr}; return m.elect,nil }
...
func TestLoanPoolSubmit(t *testing.T){
    ...
    // zero amount error
    if _,err:=lp.Submit(addrWith(2), addrWith(3), StandardLoan, 0, "x"); err==nil { t.Fatalf("expected amount zero error") }

    // long desc error
    long := make([]byte, 300); for i:=range long{ long[i]='a'}
    if _,err:=lp.Submit(addrWith(2), addrWith(3), StandardLoan, 100,long); err==nil { t.Fatalf("expected desc too long") }

    // elector error propagates
    badElect := mockElector{randErr: errors.New("boom")}
    lpBad := NewLoanPool(nil, led, badElect, lpCfg)
    if _,err:=lpBad.Submit(addrWith(2), addrWith(3), StandardLoan, 100, "ok"); err==nil { t.Fatalf("expected elector error") }
    ...
    if len(raw)==0 { t.Fatalf("proposal not stored") }
}
...
func TestLoanPoolVote(t *testing.T){
    ...
    // authority approve
    if err:=lp.Vote(authAddr, id, true); err!=nil { t.Fatalf("auth vote err %v",err) }

    // duplicate vote
    if err:=lp.Vote(authAddr, id, false); err==nil { t.Fatalf("expected duplicate vote error") }

    // public approve triggers pass
    if err:=lp.Vote(pubAddr, id, true); err!=nil { t.Fatalf("pub vote err %v",err) }
    // fetch and ensure status Passed
    raw,_ := led.GetState(proposalKey(id)); var p Proposal; _=json.Unmarshal(raw,&p)
    if p.Status!=Passed { t.Fatalf("status %v want Passed", p.Status) }
}
...
func TestLoanPoolDisburse(t *testing.T){
    ...
    if err:=lp.Disburse(id); err==nil { t.Fatalf("expected transfer fail") }
    led.txErr = nil
    if err:=lp.Disburse(id); err!=nil { t.Fatalf("disburse err %v",err) }
    raw,_ := led.GetState(proposalKey(id)); var p Proposal; _=json.Unmarshal(raw,&p)
    if p.Status!=Executed { t.Fatalf("status not executed") }
}
...
func TestLoanPoolTick(t *testing.T){
    ...
    raw,_ := led.GetState(proposalKey(id)); var p Proposal; _=json.Unmarshal(raw,&p)
    p.Deadline = time.Now().Add(-time.Hour).Unix(); led.SetState(proposalKey(id), p.Marshal())

    lp.Tick(time.Now())
    raw,_ = led.GetState(proposalKey(id)); _=json.Unmarshal(raw,&p)
    if p.Status==Active { t.Fatalf("expected status updated, got Active") }
}
