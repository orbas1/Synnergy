...
type scMem struct{ mu sync.RWMutex; kv map[string][]byte }

func newScMem() *scMem { return &scMem{kv: make(map[string][]byte)} }

func (m *scMem) SetState(k,v []byte) error { m.mu.Lock(); m.kv[string(k)] = append([]byte(nil), v...); m.mu.Unlock(); return nil }
func (m *scMem) GetState(k []byte) ([]byte,error){ m.mu.RLock(); defer m.mu.RUnlock(); return m.kv[string(k)], nil }
func (m *scMem) DeleteState(k []byte) error { m.mu.Lock(); delete(m.kv,string(k)); m.mu.Unlock(); return nil }
...
func (m *scMem) HasState(k []byte) (bool,error){ m.mu.RLock(); defer m.mu.RUnlock(); _,ok:=m.kv[string(k)]; return ok,nil }
...
type stubToken struct{ id TokenID; moved map[string]uint64 }
...
func addr(byteVal byte) Address { var a Address; for i:=0;i<20;i++{ a[i]=byteVal }; return a }

func mustRegisterToken(tkn *stubToken){ // insert into global registry used by GetToken
    r := getRegistry()
    r.mu.Lock(); if r.tokens==nil{ r.tokens = make(map[TokenID]*BaseToken)}; r.mu.Unlock()
    ...
    r.mu.Lock(); r.tokens[tkn.id] = bt; r.mu.Unlock()
}
...
func TestOpenChannel(t *testing.T){
    led := newScMem(); InitStateChannels(led)
    tok := newStubToken(1); mustRegisterToken(tok)
    a := addr(0xAA); b := addr(0xBB)

    tests := []struct{ name string; amtA, amtB uint64; expectErr bool }{
    ...
    for i,tc := range tests{
        ...
        if (err!=nil)!=tc.expectErr{ t.Fatalf("case %s err %v", tc.name, err) }
    }
}
...
func TestFinalize(t *testing.T){
    ...
    if ok,_ := led.HasState(chKey(id)); ok { t.Fatalf("channel state not deleted after finalize") }
}
...
func TestChallengeErrors(t *testing.T){
    ...
    higher := ch; higher.Nonce = 4 // lower nonce
