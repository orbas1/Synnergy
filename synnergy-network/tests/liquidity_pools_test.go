...
func (m *mockToken) Transfer(from,to Address,amt uint64) error {
    ...
    if m.bal[from] < amt { return fmtError("insufficient") }
    m.bal[from]-=amt; m.bal[to]+=amt; return nil
}
...
func (l *ammLedger) MintLP(addr Address, pid PoolID, amt uint64) error {
    ...
    if l.lp==nil { l.lp=make(map[Address]map[PoolID]uint64) }
    if l.lp[addr]==nil { l.lp[addr]=make(map[PoolID]uint64) }
    l.lp[addr][pid]+=amt; return nil
}
func (l *ammLedger) BurnLP(addr Address, pid PoolID, amt uint64) error {
    l.mu.Lock(); defer l.mu.Unlock()
    if l.lp==nil||l.lp[addr]==nil||l.lp[addr][pid] < amt { return fmtError("burn") }
    l.lp[addr][pid]-=amt; return nil
}
