...
func (m *memLedger) SetState(k, v []byte) error { m.mutex.Lock(); m.kv[string(k)] = append([]byte(nil), v...); m.mutex.Unlock(); return nil }
func (m *memLedger) GetState(k []byte) ([]byte, error) { m.mutex.RLock(); defer m.mutex.RUnlock(); return m.kv[string(k)], nil }
func (m *memLedger) HasState(k []byte) (bool, error) { m.mutex.RLock(); defer m.mutex.RUnlock(); _, ok := m.kv[string(k)]; return ok, nil }
func (m *memLedger) DeleteState(k []byte) error { m.mutex.Lock(); delete(m.kv, string(k)); m.mutex.Unlock(); return nil }
...
type memIter struct{ keys, vals [][]byte; idx int }
func (it *memIter) Next() bool  { it.idx++; return it.idx < len(it.keys) }
func (it *memIter) Key() []byte { if it.idx>=0&&it.idx<len(it.keys){return it.keys[it.idx]} ;return nil }
func (it *memIter) Value() []byte { if it.idx>=0&&it.idx<len(it.vals){return it.vals[it.idx]} ;return nil }
...
type stubBC struct{ cnt int }
func (b *stubBC) Broadcast(topic string, msg interface{}) error { b.cnt++; return nil }
...
func addr(b byte) Address { var a Address; for i := 0; i < 20; i++ { a[i] = b }; return a }
...
func TestRegister(t *testing.T) {
    led := newMemLedger()
    sc := &SidechainCoordinator{Ledger: led, Net: &stubBC{}}
...
func TestVerifyWithdraw(t *testing.T) {
    ...
    var txRoot [32]byte; copy(txRoot[:], rootBytes)
